        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4">
          ...
          <div className="mt-2 px-1">
            <a
              ...
              className="flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-sm text-zinc-400 transition-colors hover:bg-white/5 hover:text-white"
            >
    ...
    try {
      const res = await fetch("/api/admin/site-config", {
        ...
        body: JSON.stringify({
          ...
          mainPaypalEmail: config.mainPaypalEmail,
        }),
      });
        ...
        <div className="space-y-5">
          ...
          <div className="rounded-xl border border-white/5 p-6" style={{ backgroundColor: "#121214" }}>
            ...
            <p className="mb-5 text-xs text-zinc-500">
              PayPal email (Friends &amp; Family) for the main site checkout. Leave empty
              to accept only Litecoin. The bot uses this to show buyers where to pay and
              to match incoming PayPal notifications.
            </p>
            <div className="flex flex-col gap-2">
              <label className="text-xs font-semibold text-zinc-400">PayPal Email (main site)</label>
              <input
                type="email"
                value={config.mainPaypalEmail}
                onChange={(e) => update("mainPaypalEmail", e.target.value)}
